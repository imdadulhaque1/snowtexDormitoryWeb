...
const MenuSelector: React.FC<MenuSelectorProps> = React.memo(
  ({ menus, roleBasedMenu, onSelectionChange, label }) => {
    ...
    // Update parent when selected menus change
    useEffect(() => {
    ...
    const handleCheckboxChange = useCallback(
      (menuId: number, subItems: Menu[] = []) => {
        const isSelected = selectedMenus.includes(menuId);

        let updatedSelection: number[];
        if (isSelected) {
          // Remove the current menu and its sub-items
          updatedSelection = selectedMenus.filter(
            (id) =>
              id !== menuId && !subItems.some((item) => item.menuId === id)
          );
        } else {
          // Add the current menu and its sub-items
          updatedSelection = [
            ...selectedMenus,
            menuId,
              ...
              menu.subItems.map((subItem) => (
                ...
                >
                  <input
                    ...
                    onChange={() => handleCheckboxChange(subItem.menuId, [])}
                    className="w-4 h-4 cursor-pointer"
                  />
